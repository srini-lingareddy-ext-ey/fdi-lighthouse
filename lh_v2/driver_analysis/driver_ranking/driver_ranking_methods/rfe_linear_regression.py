...
class RFELinearRegressionRanking(AbstractRankingMethod):
    """
    Ranking method that uses Recursive Feature Elimination (RFE) with Linear Regression.

    This class implements a driver ranking approach based on Recursive Feature Elimination (RFE)
    with a Linear Regression model as the base estimator. It performs multiple iterations,
    ...
    method_params : dr_params.RFELinearRegressionRankingParams
        Parameters for the RFE ranking method, including number of iterations.
    ...
    def __init__(
        ...
        assert isinstance(method_params, dr_params.RFELinearRegressionRankingParams), (
            f'{self.name()} Method Params not of correct type, Expected '
            f'{dr_params.RFELinearRegressionRankingParams}, Params were of type {type(method_params)}'
        )
    ...
    def apply(self) -> ArrayF:
        ...
        This method uses a Linear Regression model as the estimator for RFE. It performs
        ...
        - A Linear Regression model is used as the estimator for RFE.
        ...
        for k in range(self.method_params.n_iter):
            ...
            # Create Linear Regression model as the estimator for RFE
